    ...
    mkc_context_t *context, mkc_option_t *mkcoptions, mkc_error_t *mkcerr)
{
  ...
  process->attr.version = NULL;
  process->attr.libraryversion = NULL;
...
mkc_process_stmt_project (mkc_process_t *process)
{
  ...
  if (process->attr.version != NULL) {
    mkc_pvar_set_str (process->pvar, MKC_C_PROJECT_VERS, process->attr.version, MKC_VCTXT_MKC);
  }
  if (process->attr.libraryversion != NULL) {
    mkc_pvar_set_str (process->pvar, MKC_C_PROJECT_LIB_VERS, process->attr.libraryversion, MKC_VCTXT_MKC);
  }
  mkc_pvar_profile_set_idx (process->pvar, pidx);
...
// ### this will need to be fixed
  ...
  *nm = '\0';
  ...
  switch (iasttype) {
    ...
    case MKC_T_ATTR_VERSION: {
      p = &process->attr.version;
      break;
    }
    case MKC_T_ATTR_LIBRARY_VERSION: {
      p = &process->attr.libraryversion;
      break;
    }
    default: {
      mkc_error_set (process->mkcerr, MKC_ERR_UNHANDLED_VALUE, 0, NULL);
...
mkc_process_int_checks (mkc_process_t *process)
{
  ...
  if (process->systype == MKC_SYS_WINDOWS) {
    process->MKC_C_OBJEXT = ".obj";
    process->MKC_C_EXEEXT = ".exe";
  }
  mkc_pvar_set_str (process->pvar, MKC_C_OBJEXT, process->MKC_C_OBJEXT, MKC_VCTXT_MKC);
  mkc_pvar_set_str (process->pvar, MKC_C_EXEEXT, process->MKC_C_EXEEXT, MKC_VCTXT_MKC);
...
mkc_process_configure_auto (mkc_process_t *process, int defzero)
{
  ...
  if (process->attr.output != NULL) {
    stpecpy (fname, fname + MKC_PATH_MAX, process->attr.output);
    tp = strrchr (fname, '/');
    if (tp != NULL) {
...
mkc_process_find_executables (mkc_process_t *process)
{
  ...
  while (chk->program != NULL) {
    ...
    while ((lidx = mkc_list_iter_next (pathlist, &iteridx)) != MKC_ITER_FINISH) {
      ...
      p = stpecpy (p, testpath + MKC_PATH_MAX, process->MKC_C_EXEEXT);
...
mkc_process_attr_clear (mkc_process_t *process)
{
  ...
  datafree (process->attr.version);
  process->attr.version = NULL;

  datafree (process->attr.libraryversion);
  process->attr.libraryversion = NULL;
